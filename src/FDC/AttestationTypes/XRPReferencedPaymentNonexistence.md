...
A confirmed request shows that a transaction meeting certain criteria (address, amount, reference, sourceAddressesRoot) did not appear in the specified block range on the XRPL chain.
...
If `firstOverflowBlock` cannot be determined or does not have a sufficient number of confirmations, defined as at least 3 (block at the tip has number of confirmations 1), the attestation request is rejected.
