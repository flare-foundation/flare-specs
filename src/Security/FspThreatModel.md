...
A second input shapes the round itself. The epoch's **canonical feed order** — the set and ordering of feeds every round prices, medians, and Merkle-roots — is derived (`rewardEpochFeedSequence`) from inflation offers (governance) plus community `RewardsOffered` (`offerRewards`, permissionless, ≥ `minimalRewardsOfferValueWei` per offer, community feeds ordered by total offered value). So for the next reward epoch an attacker can add arbitrary `feedId`s and reorder the non-inflation feeds. It's deterministic on-chain (all nodes agree — no consensus split) and per-feed medians stay independent (an unpriced attacker feed resolves to empty, not poisoning real ones), so the impact would be availability/resource, not soundness: every added feed is fetched from the feed value provider, medianed, and added as a Merkle leaf on every round for the whole epoch, across all voters. There is no feed-count cap in the canonical-order builder, **but the per-offer minimum gates it out of practicality**: `minimalRewardsOfferValueWei` is 100,000 FLR per offer on Flare (200,000 SGB on Songbird), charged per feed and paid into the reward pool — so injecting feeds costs ≥ 100,000 FLR each. Like buying signing-policy weight, the mechanism is real but not cost-effective at current parameters; it would only matter if that minimum were lowered.

One related sharp edge sits in the reveal encoding: `getFeedValuesForEpoch` maps the feed value provider's response onto the canonical feed list **positionally** — there's a length guard but no `feedId`-keyed match, so it trusts the provider to return exactly one value per requested feed, in order. Because the feed list is partly offer-shaped, a provider that mishandles an unknown feed could miss the round (count mismatch → throw) or, worse, encode values against the wrong feeds (silent misaligned reveal). It's behind the operator-trusted provider and still gated by the per-feed offer cost, but worth hardening regardless: match returned values to requested feeds by `feedId` and treat a missing feed as empty.

Residual: whether every aggregation path (median, random, weight sum) consumes only the eligibility-filtered set; determinism of duplicate/out-of-order/tie handling across nodes; precision of the median/weight arithmetic; the feed-list inflation above; and the positional reveal-encoding trust just noted.
