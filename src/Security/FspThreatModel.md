...
### 5.8 fast-updates
A separate subprotocol with its own client (`fast-updates`, Go) and contracts (`FastUpdater`, `FastUpdateIncentiveManager`). It reads chain state directly over RPC (`FilterLogs`), not through the indexer, so it's on its own ingestion path. `submitUpdates()` is a gated entry point ([§2.2](#22-on-chain-inputs-actionable-gated-and-protocol-emitted)) — callable by anyone, but a success needs a won sortition credential plus a registered provider's signature in the window, else it reverts. Its events are protocol-emitted: `FastUpdateFeedsSubmitted` per submission, `FastUpdateFeeds` per round (published by the flare daemon on epoch rollover). The one anonymous lever is paying `FastUpdateIncentiveManager` to raise the update rate — cadence and the out-of-scope reward side, not soundness. Residual: the RPC ingestion path (no indexer to bound result sets or apply the calldata cap) and the correctness of sortition-credential verification.
...
- **Goroutine crash-resistance.** A panic on a listener or worker goroutine in either Go client silently halts that voter. Whether each goroutine entry point recovers is cheap insurance against future schema/library changes — a uniform property, not per-path.
