...
- The on-chain digest verified at `verify` is identical to the one signed at `relayAndVote`; both follow the [signature-computation rule](../Concepts.md#signature-computation).
...
`Verified` for this sub-workflow. Higher-level workflows that invoke `Fdc2Attestation` consume the verified proof and may move their own state machine forward (e.g. [MachineRegistration](../../FCC/Workflows/MachineRegistration.md), [XrpPayment](../../PMW/Workflows/XrpPayment.md)).
