...
Like the [FDC](../FDC/Introduction.md), FDC2 is an enshrined oracle that imports validated external data onto Flare; unlike the FDC, it uses TEE machines as the trust anchor instead of on-chain voting.
...
2. Each [data provider](../Terminology/Roles.md#data-provider) (and cosigner, if set) picks up the instruction off-chain, validates `(data, source)` against the request, and produces an attestation response.
...
3. [`PMWMultisigAccountConfigured`](Reference/AttestationTypes/PMWMultisigAccountConfigured.md): confirms that a multisig account on an external chain is correctly configured for PMW use.
...
- `proofOwner`: address authorized to publish the proof; the zero address denotes a public proof that anyone may publish.
- `thresholdBIPS`: data-provider voting threshold for this request, in basis points; must be at least $4000$ ($40\%$ of weight). If set to `0`, the [TEE proxy](../FCC/Reference/Components/Proxy.md#signing-threshold-resolution) uses the signing policy default value.
...
Cosigner or TEE signature fields may be empty when not applicable (e.g. no cosigners are specified).
...
1. Checks that `attestationType` and `sourceId` match the expected values.
