...
An initial extension, known as the *system extension*, operates certain TEE protocols necessary for the functionality of the TEE infrastructure. 
...
By nature, extensions are designed to be flexible, and thus are defined by only two features:
...
To initialise a new extension, a Flare user issues calls the function `register(teeExtensionStateVerifier,teeExtensionInstructionsSender)` on the `teeExtensionRegistry` smart contract, with parameters $\mathrm{teeExtensionStateVerifier}$ and $\mathrm{teeExtensionInstructionSender}$ denoting the [?] and the address from which instructions will be sent to participating TEEs. 
...
The initialization call does not register any TEEs to the extension. 
These must be registered separately, as described in [cite TEE ownership].
