...
The initialization call does not register any TEEs to the extension, which must be registered separately as described [here](TeeOwnership.md).
...
-  `proposeNewOwner(extensionId, address_new)`: Proposes a new owner address `address_new` for the extension. To complete the change of ownership, the function `confirmOwnership(extensionId)` must be called from `address_new` after the function is called, at which point ownership of the extension is transferred.
