...
## Project Workflow
The owner of a project is the address with permissions to set up wallets, cosigners, and keys for PMWs on the project.
The workflow beginning from instantiating a project to running a production PMW follows a sequence of steps:
...
This determines parameters $(n,k)$ for a threshold signature, with $k$ of the $n$ cosigner addresses required to sign a payment instruction before a TEE machine executes it.

Once the cosigner addresses and threshold for a wallet are set, they can not be changed.
The TEE machines store immalleable cosigner information as metadata alongside the keys for the wallet so that they can check whether each instruction has been appropriately signed, preventing issues with [cosigner enforcement](Instructions.md/Cosigners).

## Data Structures
Formally, the components of a PMW are a sequence of nested data structures, with each one being a field within the above structure.
...
Users interact with these data structures using the TEE Wallet and Project management contracts, whose calls are laid out in the following section.

### Project Data Structures
For a Flare user to initialize a PMW, they must first have a project oin place.
Thus, initializing a project data structure is the first part of registering a PMW.
To do so, the user calls the `createProject(opType, submitAddress)` function at the `TeeProjectManager` contract.
...
- `submitAddress`: The (Flare) address from which the user can submit payment instruction transactions for wallets on the project. Typically, this will be different to the owner address.
...
- `tees`: A list of TEE machine IDs on which the key exists. This field is updated using a `TeeKeyExistence` [proof](KeyManagement.md). 
...
- `createProject(opType, submitAddress)`: Initializes a new project, generating a project ID and marking the address that called the function as the project owner. The functionality of the project is set to `opType` and `submitAddress` is the address that has permission to submit payment instructions.
