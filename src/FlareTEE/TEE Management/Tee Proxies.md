...
Direct actions are created internally by the proxy for `TEE_INFO`, `KEY_INFO`, `UPDATE_POLICY`, and `TEE_BACKUP` operations. These are not submitted through the external API but are instead generated by the proxy's own periodic services and policy update logic.
...
- **Key data store**: ([`walletId`, `keyId`] $\rightarrow$ `keydata`), tracks the key data for any keys stored in the TEE for [PMW](../Extensions/PMW/PMW.md) operations. Stored in-memory and refreshed on each sync cycle. Entries for deleted keys are cleared when absent from the TEE response.

### Redis-Backed Stores

- **Action store**: ([`actionId`, `submissionTag`] $\rightarrow$ `actionData`), tracks the data for a given action identity. TTL: $30$ days.
- **Action result store**: ([`actionId`, `submissionTag`, `result`] $\rightarrow$ `actionResult`), tracks the result for a given action identity. TTL: $14$ days for standard results, $30$ minutes for submit-type results.
- **Signing policy store**: (`rewardEpochId` $\rightarrow$ `signingPolicyData`), tracks the signing policy data for a given reward epoch.
- **Backup store**: (`backupIdHash` $\rightarrow$ `backupData`), tracks the data related to key backups stored by the TEE machine. Backup packages are extracted from the TEE machine each time `TEE_INFO` is called. TTL: $8$ days.
- **Backup index store**: ([`walletId`, `keyId`] $\rightarrow$ `backupIdHash`), maps a wallet key to the latest backup ID hash, enabling lookup of the most recent backup for a given key. TTL: $8$ days.

### Last Attestation
The last available attestation is updated through the `TEE_INFO` action. This is updated by the TEE proxy every $10$ seconds, which generates a random challenge and calls the action.
...
Key data is stored in-memory and refreshed on each sync cycle. Entries for deleted keys are cleared when absent from the TEE response.
