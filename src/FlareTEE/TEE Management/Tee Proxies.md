...
After a successful [`UPDATE_POLICY`](../commands/F_POLICY--UPDATE_POLICY.md) action, the proxy triggers fresh [`TEE_BACKUP`](../commands/F_GET--TEE_BACKUP.md) actions for keys currently stored on the TEE machine, so that backup packages are refreshed under the new signing policy.

## Instruction Threshold Resolution
The proxy resolves the effective cosigner set and any instruction-specific threshold overrides according to the instruction family.
...
3. For [`PROVE`](../commands/F_FDC2--PROVE.md), the data-provider threshold is taken from `thresholdBIPS` in the FDC2 request header, and the current signing-policy default is used if the value is zero.
4. For other instructions, the proxy uses the `cosigners` and `cosignerThreshold` fields carried by the instruction itself.

These rules determine how the proxy evaluates incoming signatures when a voting process is initialized and advanced.
...
Different instruction families may use different limits, especially when a restore flow carries a larger encrypted payload.
Each deployment should publish the supported limits of its proxy APIs.
An instruction that exceeds a supported limit is invalid for that proxy deployment and must not start or advance a voting process.
...
These system operations are therefore *direct-only* and must not be submitted through `POST /instruction`.
