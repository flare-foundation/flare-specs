...
## Penalization
Thus far, correct provider behaviour has been assumed. That is, proper functioning of the FTSO process requires that the data revealed in the reveal phase by each provider correctly hashes to their commitment published in the commit phase. Additionally, it was assumed in the signing and finalization phases that only one root receives enough signatures to be finalized, implicitly requiring that each signer does not sign multiple messages. Finally, it is assumed that providers complete their responsibilities in a timely manner. 
