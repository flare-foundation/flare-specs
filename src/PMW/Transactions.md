...
A user authorized to issue payments on a PMW account calls:
...
- `maxFee`: maximum fee the user is willing to pay.
...
`pay` returns the assigned `paymentId`.

## How Payment Instructions Reach TEE Machines

Payment instructions are relayed to the TEE machines by the `TeePayments` contract.
For each `pay` (or `reissue`) call, the `TeePayments` contract:

1. Retrieves the `walletId` from `(sourceId, accountAddress)`, calls `FlareTeeManager.receivingTeesAndKeys(walletId)` to obtain the `(teeId, keyId)` pairs that should sign the transaction, and resolves the effective fee schedule via the `TeePaymentsFeeScheduleManager` contract.
2. Builds a [`PaymentInstructionMessage`](Reference/Types/Payment.md#paymentinstructionmessage), which encodes:
   ...
   - `sourceId`, `senderAddress`, `recipientAddress`, `tokenId`, `amount`, `maxFee`, `projectId`, `paymentReference`.
...
Some UTXO chains allow several payments inside a single transaction.
For these chains, `TeePaymentsUtxo` opens a batch on the first payment to a previously inactive wallet and accumulates further payments until either:

- the number of payments in the batch reaches `batchSize`, or
...
Batch settings are per `(walletId, account)`, controlled by the wallet owner calling:

```solidity
TeePaymentsUtxo.setBatchSettings(PMWMultisigAccount account, uint64 batchSize, uint64 batchDurationSeconds)
```
...
3. If $|\mathrm{T}_\mathrm{list}| < S$ at time $t + d$, the batch closes with whatever payments it has.

## Reissuing a Payment

A stuck payment can be reissued with the same payment data but a different fee schedule:

```solidity
TeePayments.reissue(PMWMultisigAccount account,
                    uint64 paymentId,
...
The caller passes the original payment's `paymentId` and the same set of payment instructions; `reissueFeeParams` provides a fresh fee schedule:
...
A negative factor value instead triggers [nullification](#nullification).

## Fee Schedules

When a payment instruction reaches a TEE machine it carries a _fee schedule_ — an ordered list of `(factor, delay)` entries.
The TEE machine signs one transaction per entry and posts them to the [TEE proxy](../FCC/Reference/Components/Proxy.md) on the delay schedule, so that higher-fee versions become available automatically if earlier ones do not confirm.
...
Note that the `delay` entries must be strictly ascending.

### Fee Calculation

For each entry the effective fee is:

$$\mathrm{fee} = \dfrac{\mathrm{factor} * \mathrm{maxFee}}{10000}$$
...
Nullificaiton is used to cancel a stuck payment.
...
Fee schedules are managed on the `TeePaymentsFeeScheduleManager` contract, with precedence `account override > project default > built-in default`:
