...
Rewards are issued as **claims**, which beneficiaries need to submit to [`RewardManager`](https://gitlab.com/flarenetwork/flare-smart-contracts-v2/-/blob/main/contracts/protocol/implementation/RewardManager.sol?ref_type=heads) smart contract to receive funds.
...
Correct protocol participation results in accruing rewards, malicious behaviour is disincentivised by applying reward **penalties**. Penalties are negative reward amounts per round, which accumulate and may potentially result in total negative reward amount for an entity for the full reward epoch. In that case, the entity receives no rewards.
...
The reward calculation process relies on transaction and event data obtained from the [FSP C-Chain indexer](https://github.com/flare-foundation/flare-system-c-chain-indexer). Below is an outline for the general procedure:

1. Retrieve all relevant data for the full reward epoch:
   - Reward epoch metadata containing supported FTSO feed configurations, reward offers events, voter registrations.
   - Data provider submissions.
2. For each sub-protocol:
   - Decode and validate submissions, assign to rounds.
   - Pool rewards from all sources, [equally distribute](Operations.md#1-integer-division-with-remainder-distribution) to rounds.
   - Compute reward claims and penalties for each round.
3. Aggregate reward claims, apply minimal conditions. For providers failing to meed the conditions, reward claims will be redirected to the burn address.
4. Produce reward Merkle tree for distribution, containing proofs for reward claiming and a hash for reward signing.
5. Once the hash is signed by majority, providers can claim rewards.

### 2.2. Fund sources

Reward funds and sourced from inflation and additional protocol-specific sources.
...
Funds are combined for each protocol and distributed equally for each round.
...
- **Signing Policy**: The policy that defines which voters are eligible and how signatures are validated for this epoch.
  - Parsed from `Relay.SigningPolicyInitialized` event.
- **Reward Offers**: Information on protocol reward amounts for the epoch. Sourced from the following events:
  - `FtsoRewardOfferManager.RewardsOffered`
  - `FtsoRewardOfferManager.InflationRewardsOffered`
  - `FastUpdateIncentiveManager.InflationRewardsOffered`
  - `FastUpdateIncentiveManager.IncentiveOffered`
  - `FdcHub.InflationRewardsOffered`
- **Ordered Feeds**: The list of FTSO feeds, ordered alphabetically:
  - Default feeds: `FtsoRewardOfferManager.InflationRewardsOffered`
  - Community feeds: `FtsoRewardOfferManager.RewardsOffered`
- **Voter Index**: Addresses and metadata for registered data providers:
  - `VoterRegistry.VoterRegistered`
  - `FlareSystemsCalculator.VoterRegistrationInfo `

TODO: specify time ranges for querying events
TODO: add links to Events reference page
...
The validation rules for protocol messages are defined as follows:
| Submission Type      | **Time Range**                                          | Duplicate Handling                       | **Allowed Sender Address** |
| -------------------- | ------------------------------------------------------- | ---------------------------------------- | -------------------------- |
...
Validation rules for finalizations:
 - $T \in [t_{\text{reveal}}(R+1), t_{\text{start}}(R+2))$ 
 - `SigningPolicy` must match the current reward epoch signing policy.
 - `ProtocolMerkleRoot.RoundId`  must match the expected round:  $r = id_\text{round}(T) - 1$.
 - Signatures sign the encoded `ProtocolMerkleRoot` hash. TODO: expand
 - Only the first valid finalization per round per sender is considered.
 - Sender could be any address.
...
- All of provider's registered validator node Ids. TODO: add reference.
