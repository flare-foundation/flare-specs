...
Rewards are distributed every $3.5$ days, which constitutes a _reward epoch_. A combined reward amount is issued to a beneficiary for all sub-protocols for the whole reward epoch. 
...
A complete list of claims is calculated after the reward epoch ends, and a combined _reward hash_ is generated by using a Merkle Tree. Once the hash is signed by weight-based majority of the participants, reward claiming becomes available on `RewardManager`.
...
Correct protocol participation results in accruing rewards, malicious behavior is disincentivized by applying reward _penalties_. Penalties are negative reward amounts per round, which accumulate and may potentially result in total negative reward amount for an entity for the full reward epoch. In that case, the entity receives no rewards.

### Minimal conditions

For FSP security, infrastructure providers are expected to participate in _all_ subprotocols, as well as staking (operating a Flare validator node). A set of minimal participation conditions is defined for each protocol that have to be met for an entity to receive rewards.
...
3. $t_{\text{start}}(r)$: Start time of round $r$. See [Voting Round](Epochs.md#voting-epoch).
...
1. **Direct** `(0)`: The beneficiary is any address. The reward is solely owed to the beneficiary. This type of claim is used for back claims of undistributed rewards for providers of funds into rewarding pools, burn claims, or other direct rewarding approaches needed in sub-protocols. 
2. **Fee** `(1)`: The beneficiary is the identity address of an eligible entity for the reward epoch only (their address is provided). The fees include WFLR delegation fees and node staking fees. The reward is solely owed to the beneficiary.
3.**WFLR** `(2)`: The beneficiary is the delegation address of an eligible entity for the given reward epoch. The amount includes the value to be distributed to the delegators according to their share in $W_D(\text{beneficiary})$
4. **Mirror** `(3)`: The beneficiary is a node ID of an eligible entity for the given reward epoch. The amount includes the value that is to be distributed to the stakers according to their share in the amount staked to the node ID.
...
#### FTSO block-latency feeds
  - Providers must submit at least $80\%$ of their expected number of updates within a reward epoch, unless they have very low weight, defined as less than $0.2\%$ of the total active weight.

#### FDC
  - Providers must be rewarded in $60\%$ of voting rounds within a reward epoch.

#### Staking
  - Providers must meet $80\%$ total uptime in the reward epoch with at least $1$ million FLR in active self-bond. 
  - To earn passes, the provider must have at least $3$ million FLR in active self-bond and $15$ million in active stake. Providers with $80\%$ total uptime and at least $1$ million FLR in active self-bond but not meeting both the $3$ million FLR active self-bond and $15$ million active stake requirements neither earn nor lose passes, and still receive eligible rewards.
...
- All providers start with $0$ passes. If a provider held passes but missed the registration for an epoch, their pass count will also reset to $0$ for the next registered epoch.
- A pass is awarded to a provider who meets _all minimum participation requirements)_ across all protocols during a reward epoch, with the number of held passes capped at $3$.
- For each protocol a provider fails to meet the minimum requirement, they lose $1$ pass. 
- If a provider holds $0$ passes in a reward epoch and loses a pass, the provider loses **all** rewards for that epoch. They will start the next epoch with $0$ passes, and are eligible for rewards if all conditions are met.
