...
- **Signing Policy**: The policy that defines which voters are eligible and how signatures are validated for this epoch.
  - Parsed from `SigningPolicyInitialized` event.
...
Validation rules for finalizations:
 1. $T \in [t_{\text{reveal}}(R+1), t_{\text{start}}(R+2))$ 
 2. `SigningPolicy` must match the current reward epoch signing policy.
 3. `ProtocolMerkleRoot.RoundId`  must match the expected round:  $r = id_\text{round}(T) - 1$.
 4. Signatures sign the encoded `ProtocolMerkleRoot` hash.
 5. Only the first valid finalization per round per sender is considered.
 6. Sender could be any address.
