...
- `owner`: the Flare address that owns and manages the extension. Changed via a two-step `proposeNewOwner` / `confirmOwnership` transfer. Ownership is gated by the extension owner allowlist on `OwnerAllowlistFacet`.
