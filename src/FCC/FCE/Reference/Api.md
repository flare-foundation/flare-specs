...
HTTP contract between a [TEE machine](../../Reference/Components/Machine.md) and its FCE process.
The TEE machine and the extension process run inside the same enclave and communicate over a local HTTP loopback.
...
**Echo requirement.** The proxy stores results keyed by `(id, submissionTag)` without cross-checking the inbound action, so the extension must echo `id`, `submissionTag`, `opType`, and `opCommand` faithfully from the inbound `Action`. A result that mismatches on any of these is stored in an unreachable slot.
...
- `2`, `3`, … : Transient/in-progress. Can be overwritten by a strictly greater transient or by a terminal status. Lets the extension signal progress toward completion.
...
- **Effect**: the TEE machine wraps it in an [`ActionResponse`](../../Concepts/Actions.md#action-responses) signed with the machine's identity key and posts it to the [TEE proxy](../../Reference/Components/Proxy.md). The proxy's result storage [override rules](#post-action) apply: a transient result can advance to a higher transient or to a terminal one; a terminal result is immutable.
...
A reference deployment runs the FCE on `http://localhost:<port>` inside the same enclave as the TEE machine.
The exact port is FCE-specific; the TEE machine reads it from its startup configuration.
Access from outside the enclave to either endpoint is not part of the contract and is blocked at the network layer in production deployments.