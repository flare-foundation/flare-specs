...
- `configConstants`: immutable wallet-side configuration mirrored at key-generation time — see [`KeyConfigConstants`](../Reference/Types/Abi/Key.md#keyconfigconstants).
  Carries the [key admins](../../Terminology/Roles.md#key-admin) (`adminsPublicKeys` and `adminsThreshold`) and the optional [cosigner](Instructions.md#cosigners) set (`cosigners` and `cosignersThreshold`).
...
4. The TEE proxy sets `submissionTag = end` so [voting](Voting.md) stays open for the full window. At the end of voting, if both share sets meet their thresholds, the proxy delivers the bundle to the destination TEE.
...
6. The TEE returns an [action response](Actions.md#action-responses) indicating success or failure and, if any holders submitted invalid shares, names them.
