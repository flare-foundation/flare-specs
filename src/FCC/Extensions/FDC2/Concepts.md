...
The signed attestation is then available from the [TEE proxy](../../Components/TeeProxy.md) and can be published on Flare.
...
1. A user submits an [`Fdc2AttestationRequest`](../../Types/Abi/Fdc2.md#fdc2attestationrequest) via `Fdc2Hub`, which routes it as an [`F_FDC2 PROVE`](Commands/Prove.md) instruction to the TEE machines selected by `Fdc2Hub` (either explicitly listed in the request or chosen randomly from the registered set; see [Request Format](#request-format)).
...
3. The provider builds a signed instruction with the [`F_FDC2 PROVE` augmentation procedure](Commands/Prove.md#augmentation-procedure): `additionalFixedMessage` holds the ABI-encoded response body; `additionalVariableMessage` holds the provider's signature over the [attestation response hash](#signature-computation).
4. The signed instruction is sent to the [TEE proxies](../../Components/TeeProxy.md) of the target machines and enters the standard [voting process](../../Operations/Voting.md).
...
6. The TEE proxy serves the resulting [`ProveResponse`](../../Types/Wire/Fdc2.md#proveresponse) as the [action result](../../Operations/Actions.md#action-results).
...
1. [`TeeAvailabilityCheck`](AttestationTypes/TeeAvailabilityCheck.md) — TEE machine availability, code integrity, and platform attestation freshness.
2. [`PMWPaymentStatus`](AttestationTypes/PMWPaymentStatus.md) — status of a PMW payment transaction on an external chain.
3. [`PMWMultisigAccountConfigured`](AttestationTypes/PMWMultisigAccountConfigured.md) — that a multisig account on an external chain is correctly configured for PMW use.
4. [`PMWFeeProof`](AttestationTypes/PMWFeeProof.md) — fee accounting for a range of payment nonces (estimated fees vs. actual on-chain fees).

The verifier-server HTTP interface that data providers run for each type is documented in [Verifier](Verifier.md).

## Request Format

A request is an [`Fdc2AttestationRequest`](../../Types/Abi/Fdc2.md#fdc2attestationrequest) containing an [`Fdc2RequestHeader`](../../Types/Abi/Fdc2.md#fdc2requestheader) and a type-specific `requestBody`.
...
- `thresholdBIPS`: data-provider voting threshold for this request, in basis points; must be at least $4000$ ($40\%$). The [TEE proxy](../../Components/TeeProxy.md#signing-threshold-resolution) treats `0` as "use the signing policy default".
...
`cosigners` and `cosignersThreshold` are passed at the `sendInstructions` call rather than in `Fdc2RequestHeader`, and the proxy extracts them from the [`TeeInstructionsSent` event](../../Types/Abi/Events/FlareTeeManager.md) for the voting process.

## Response Format

An attestation response has three ABI-encoded parts:

- [`Fdc2ResponseHeader`](../../Types/Abi/Fdc2.md#fdc2responseheader) — extends the request header with cosigner fields and a timestamp.
...
The [`F_FDC2 PROVE`](Commands/Prove.md) action result is the [`ProveResponse`](../../Types/Wire/Fdc2.md#proveresponse) struct.
...
The on-chain [`Proof`](../../Types/Abi/Fdc2.md#proof) struct for each attestation type wraps a per-type `(header, requestBody, responseBody)` triple with an [`Fdc2Signatures`](../../Types/Abi/Fdc2.md#fdc2signatures) bundle composed of [`Signature`](../../Types/Abi/Common.md#signature) entries.

`ProveResponse` → `Proof` mapping:

| `ProveResponse` field | `Proof` field | Conversion |
|---|---|---|
| `ResponseHeader` | `header` | ABI-decode into [`Fdc2ResponseHeader`](../../Types/Abi/Fdc2.md#fdc2responseheader). |
...
| `TEESignature` | `signatures.teeSignatures` | Split the raw bytes into [`(v, r, s)`](../../Types/Abi/Common.md#signature) and wrap in a one-element array. |
...
4. Validates attestation-specific fields (e.g. code hash and platform for [`TeeAvailabilityCheck`](AttestationTypes/TeeAvailabilityCheck.md), account configuration for [`PMWMultisigAccountConfigured`](AttestationTypes/PMWMultisigAccountConfigured.md)).
