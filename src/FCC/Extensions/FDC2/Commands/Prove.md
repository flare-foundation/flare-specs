...
The request header's `thresholdBIPS` field overrides the signing policy's default data-provider voting threshold for this instruction; a value of $0$ falls back to the policy default. See [Signing Threshold Resolution](../../../Components/TeeProxy.md#signing-threshold-resolution).
...
Before signing, the [relay client](../../../Components/RelayClient.md) populates `additionalFixedMessage` and `additionalVariableMessage`:
