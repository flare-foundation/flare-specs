...
- registered as a system instructions sender (`Instructions.isSystemInstructionsSender(msg.sender)`): any op-type, any extension.
- equal to the destination extension's registered instructions sender (`ExtensionManager.getExtensionInstructionsSender(extensionId)`): non-[system op-types](../../FCE/Concepts.md#system-vs-custom-extensions) only.
...
The [system extension](../../FCE/System.md) (`extensionId == 0`) cannot have its own registered instructions sender: extension id $0$ is unassignable, and `setExtensionContracts` rejects `_extensionId == 0`.
...
- For non-system operations, every destination TEE machine is in `PRODUCTION` status.
...
Three allowlists on `OwnerAllowlistFacet` (see [Concepts/Machines](../../Concepts/Machines.md#owner-allowlist) for what the roles mean):

- **machine owner** (per-extension): gates `register`, `proposeNewOwner` / `confirmOwnership` on machines.
- **wallet [project owner](../../../Terminology/Roles.md#project-owner)** (per-extension): gates `createProject`.
- **[extension owner](../../../Terminology/Roles.md#extension-owner)** (global): gates `register`, `proposeNewOwner` / `confirmOwnership` on extensions.
...
| `governanceHash` | Commits machine to a specific extension-governance signer set. |
...
- `teePublicKey`, `initialTeeId` (reserved for future use), `initialSigningPolicyId`.
- `codeHash`, `platform`.
...
- `pause(teeId)`: see [Concepts/Machines](../../Concepts/Machines.md#statuses) for the per-caller transition rules.
...
`MachineEmergencyPauseFacet` is a per-extension emergency stop, separate from per-machine [statuses](../../Concepts/Machines.md#statuses) and per-wallet pausing.
While an extension is emergency paused, [`sendInstructions`/`sendSystemInstructions`](#sending-instructions) reject every dispatch to its machines with `EmergencyPauseActive`.
Machine statuses, the active sets, and the read getters are untouched, so off-chain consumers must also check `isExtensionEmergencyPaused(extensionId)`.
Clearing the flag restores dispatch immediately.
...
After unpause, a global grace window (default $\sim 2\,\mathrm{h}$) blocks only the third-party expired availability branch of [`pause(teeId)`](#management-calls) (revert `EmergencyProtectionActive`) so owners can refresh attestations before other users can suspend machine in `PRODUCTION`.
The window covers the longer of the machine's own extension and the [system extension](../../FCE/System.md) (ID 0), since an availability refresh needs both an FCE attestation and an FDC2 attestation routed to system extension machines.
Owner-initiated `pause`, disabled-version pause, and `pauseWithProof` are unaffected.
...
| Field | Description |
|---|---|
| `owner` | Current project owner address; changes only through `proposeNewOwner`/`confirmOwnership`. |
...
| `adminsPublicKeys`, `adminsThreshold` | The [key admin](../../../Terminology/Roles.md#key-admin) public keys and $k$-of-$n$ threshold; set with `setAdmins` and finalized when the wallet leaves `CREATED`. |
| `cosigners`, `cosignersThreshold` | Optional [cosigner](../../Concepts/Instructions.md#cosigners) set and its threshold; set with `setCosigners` and finalized at close. |
...
Emits a `KEY_DATA_PROVIDER_RESTORE` instruction.
See [Concepts/Keys](../../Concepts/Keys.md#key-restoration) for the recovery procedure.
