...
This directory documents the _system operations_: those carrying the `F_` op-type prefix that every TEE machine processes regardless of which [FCE](../../Extensions/README.md) it is registered to.
...
Each operation is processed either as a [_direct action_](../Actions.md#direct-actions) (issued by the [TEE proxy](../../Components/TeeProxy.md) without voting) or an [_instruction action_](../Actions.md#instruction-actions) (issued by [signers](../Instructions.md#signers) and gated by [voting](../Voting.md)).
...
The [system extension](../../Extensions/SystemExtension.md) registers additional `F_`-prefixed operations for its own applications.
These are application operations rather than infrastructure, so they live alongside the application's spec:

- [PMW operations](../../Extensions/PMW/Commands/README.md) — `F_XRP PAY`, `F_XRP REISSUE`.
- [FDC2 operations](../../Extensions/FDC2/Commands/README.md) — `F_FDC2 PROVE`.

Custom [FCEs](../../Extensions/README.md) register their own operations under non-`F_` op-type prefixes; those are defined and documented by each FCE and are out of scope for this directory.
