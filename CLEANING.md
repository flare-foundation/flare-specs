...
- [x] `src/FCC/Images/`
- [x] `src/FCC/Concepts/{Actions,Instructions,Voting,Rewarding,README,TrustModel}.md`
- [x] `src/FCC/Reference/Components/RelayClient.md`
- [x] `src/FCC/Architecture.md`
- [x] `src/FCC/README.md`
- [x] `src/Utilities/Signing.md`
- [x] `src/Terminology/`

All other files under `src/FCC/`, `src/PMW/`, and `src/FDC2/` have completed phases 1 and 2; some are partially through phase 3.

## Not Yet Cleaned

Files outside the protocols listed above (`src/FCC/`, `src/PMW/`, `src/FDC2/`, the cleaned `src/Utilities/`, `src/Terminology/`) have not been through any cleaning phase.
...
Per-page leaf cleaning of `src/FCC/Reference/Components/{Machine,Proxy}.md`, `src/FCC/Reference/Operations/F_*.md`, `src/PMW/Concepts.md`, `src/PMW/Transactions.md`, `src/FDC2/Concepts.md`, `src/FDC2/Verifier.md`, `src/FCC/FCE/Concepts.md`, `src/FCC/FCE/System.md`.
...
Known files to fix: `src/FCC/FCE/Concepts.md`, `src/FCC/FCE/Workflows/Configuration.md`, and the inbound link from `src/FCC/Reference/Operations/Instructions.md` once `src/FCC/FCE/Concepts.md` gains a stable anchor.

A _system instructions sender_ (governance-registered, allowed to send `F_` op-types and to call `sendSystemInstructions`) is currently only mentioned in passing in `src/FCC/FCE/Concepts.md`.
Give it a dedicated subsection with a stable anchor so it can be linked on first mention from `src/FCC/Reference/Operations/Instructions.md` and elsewhere.

Once those anchors exist, add links in `src/FCC/Reference/Operations/Instructions.md`'s `### Instructions Senders` bullets:
...
Spot-check `src/PMW/Workflows/XrpPayment.md`, `src/FDC2/Reference/AttestationTypes/PMW*.md`, and `src/FCC/FCE/Workflows/Instructions.md`.
...
- `src/FCC/Reference/Contracts/FlareTeeManager.md:84` re-italicizes _machine owner_ etc. when listing which contract functions each allowlist gates. That function-gating detail is legitimate reference content, but it should link the role to `Roles.md` instead of reading as a fresh definition.
- `src/FCC/Reference/Components/RelayClient.md:11` uses _Data provider_ as a config term; confirm it links `Roles.md#data-provider` on its cleaning pass.
...
- `src/FCC/Concepts/Keys.md:104, 106` — holder public keys ($\mathrm{pk}_i$) and the Shamir backup encryption.
- `src/FCC/Reference/Operations/F_WALLET.md:112-116` and `src/FCC/Workflows/VrfProof.md:46, 58` — VRF math (`sk`, `pk`).
- `src/FCC/Reference/Contracts/VrfVerifier.md:25, 28, 68-71, 87` — VRF verification equations.
...
- _JSON encoding_ — appears as a noun phrase in `src/FCC/Reference/Operations/Actions.md` lines 20, 47; rewrite as "JSON-encoded" wherever grammatically possible, or accept the noun form only where unavoidable.
- _JSON marshaled_, _marshalled_, _marshaled_ — Go-specific jargon; replace (`src/FCC/Reference/Components/Proxy.md` lines 164, 170; `src/FCC/Reference/Types/Wire/Action.md` line 73).
...
- `src/FCC/FCE/README.md` and `src/FCC/FCE/Concepts.md`.
- `src/FCC/Workflows/README.md` (line 65)
...
Spot-check the rest of the docs (`src/FCC/Workflows/`, etc.) for inconsistent usage (_produce_, _assemble_, _construct_, _create_, _make_, ...) and converge on _build_.
...
- Pushing the byte-layout / size details back into `FSP/Encoding.md` (already the source of truth for these layouts) and leaving `src/Utilities/Signing.md` with a one-sentence pointer.
- Or, if a dedicated FSP/FCC page emerges for the relay-format signatures (e.g. as part of `FSP/Relay.md`, see Outside FCC/), move the `ECDSASignatureWithIndex` discussion there and have `src/Utilities/Signing.md` link to it.

The same refactor likely applies to the inbound FCC mentions of "relay format" in `src/FDC2/Concepts.md` and `src/FDC2/Reference/Types/Abi/Fdc2.md` — they should defer to the canonical layout doc instead of repeating "encoded in relay format using the signing policy".

`SignatureType0` is deprecated and should not be mentioned anywhere in the repo; this includes the `src/Utilities/Signing.md` bullet referenced above. See the matching FSP-cleaning TODO for the full removal scope.

#### Extract reference content from `src/FCC/FCE/Concepts.md` into `src/FCC/Reference/Contracts/FlareTeeManager.md`

Pull the management-call list (the `register`, `setExtensionContracts`, `addTeeVersion`, `disableCodeHashPlatform`, `addSupportedKeyTypes`, `removeSupportedKeyTypes`, `proposeNewOwner`, `confirmOwnership`, `sendInstructions`, `sendSystemInstructions`, `addSystemSupportedPlatforms`, `addSystemSupportedKeyTypesAndSigningAlgos`, `registerSystemInstructionsSenders`, `unregisterSystemInstructionsSenders` entries) into a new "Extension Management" section under `src/FCC/Reference/Contracts/FlareTeeManager.md`.
Leave `src/FCC/FCE/Concepts.md` with the concept-level prose (extension data model, system-vs-custom distinction, lifecycle narrative, instructions-senders concept) and cross-links to the new Reference section.

#### Sweep `Concepts.md` files for content fit

Concepts pages exist at `src/FCC/FCE/Concepts.md`, `src/PMW/Concepts.md`, `src/FDC2/Concepts.md`, and (outside FCC) `Terminology/Concepts.md`.
Review each in turn:

- Does any section sit at the wrong level (e.g. an extension-framework concept in `src/PMW/Concepts.md` that belongs in `src/FCC/FCE/Concepts.md`, or an FCC-wide concept in `src/FCC/FCE/Concepts.md` that belongs in `Terminology/Concepts.md`)?
...
- `src/FCC/Reference/Operations/Instructions.md` — the sending-instructions pipeline (user → instructions sender → `FlareTeeManager` → event → signers → TEE proxies → voting → action).
- `src/FCC/Reference/Operations/Voting.md` — proxy flow, vote-box lifecycle, threshold/end outcomes.
- `src/FCC/Reference/Operations/Rewarding.md` — the vote-hash chain extension and the verifier reconstruction procedure.
- `src/FCC/Reference/Components/Proxy.md` — the three processing queues and the action-result handling pipeline.
...
The C-chain indexer (mentioned in `src/FCC/Architecture.md`'s deployment topology and `src/FCC/Reference/Components/RelayClient.md`'s relay flow) and Redis (proxy state store) are operator-run dependencies with no dedicated spec.
...
The external signer used by the relay client is currently only mentioned by name; if there is a stable HTTP contract for it (`POST /sign`, `POST /decrypt`, `GET /id` are observed in code), surface it explicitly — either as a section in `src/FCC/Reference/Components/RelayClient.md` or as a sibling `Components/ExternalSigner.md`.

#### Document async-result behavior in `F_XRP PAY` / `F_XRP REISSUE`

These two PMW commands are registered with `immediateResult=false` (`tee-node/internal/router/routers.go:49-50`), so their `ActionResult.status` flows `2` (in-progress) on `threshold` → `1` (success) on `end`. All other system commands return `status=1` directly on `threshold`. `src/FCC/Reference/Operations/Actions.md` keeps the `status=2` description generic; surface this command-specific behavior in `src/PMW/Reference/Operations/Pay.md` and `src/PMW/Reference/Operations/Reissue.md` as part of the `Action result` section when those files are cleaned.
...
- Surface `directBackup` / `directRestore` and the path-list authorization in `src/FCC/Reference/Contracts/FlareTeeManager.md`.
- Add `KEY_DIRECT_BACKUP` and `KEY_DIRECT_RESTORE` to `src/FCC/Reference/Operations/F_WALLET.md` alongside the existing key commands; reflect the post-`cd32db53` payload (no `destinationNonce` on backup; nonce binding only on restore).
- Decide whether [`src/FCC/Workflows/TeeBackup.md`](src/FCC/Workflows/TeeBackup.md) and [`src/FCC/Workflows/KeyRestore.md`](src/FCC/Workflows/KeyRestore.md) document the direct alternative inline, gain new sibling workflows (`KeyDirectBackup.md` / `KeyDirectRestore.md`), or both. Mirror in the [TLA+ formal models](#tla-formal-models).
...
- `src/FCC/Reference/Types/Abi/Instruction.md` and `src/FCC/Reference/Types/Wire/Instruction.md` — add leading `chainId` to `Instruction`.
- `src/FDC2/Reference/Types/Abi/Fdc2.md` (and `Wire/Fdc2.md`) — add leading `chainId` to `Fdc2ResponseHeader`; note the verifier requirement.
- `src/FCC/Reference/Operations/F_REG.md`, `src/FCC/Reference/Operations/F_WALLET.md` — describe the `(domain-tag, chainId, payload)` hashing shape on the relevant signatures (this is reference-level; `src/FCC/Concepts/Machines.md § Attestation` stays at concept level).
- `src/FCC/Workflows/MachineReplication.md` (TeeUpgrade flow), the TeeUpgrade event family in `src/FCC/Reference/Contracts/FlareTeeManagerEvents.md`, and the upgrade-related parts of `src/FCC/Reference/Contracts/FlareTeeManager.md` — surface the new hash preimage.
...
- Update `src/FCC/Concepts/Wallets.md` lifecycle to reflect the same model.
- Touch `src/FCC/Workflows/WalletSetup.md` (and any wallet-pause workflow that emerges).
- Optionally fold the WalletProjectPauseFacet management calls into a dedicated subsection of `src/FCC/Reference/Contracts/FlareTeeManager.md` (the lifecycle line currently links them inline).
...
The reward-distribution flow — how participation in TEE voting and signing is attributed and reconstructed off-chain into per-data-provider rewards — was considered for `src/FCC/Workflows/` but rejected: the mechanism lives in the FSP, and the FCC side already documents the per-vote receipt structure in [`src/FCC/Concepts/Rewarding.md`](src/FCC/Concepts/Rewarding.md). Cover the full reward-distribution workflow in `src/FSP/` rather than `src/FCC/` when the FSP cleaning pass picks it up.

### Drive-by Phase 1 verification of `src/FCC/Reference/Operations/F_*.md`
...
These items concern files or behavior outside `src/FCC/`; recorded here for visibility until a cross-protocol or per-protocol cleaning pass picks them up.

### READMEs

- Rename `Introduction.md` → `README.md` in the other protocol roots (`src/FSP/`, `src/FDC/`, `src/FTSO/`). Each plays the same landing-page role that `src/FCC/README.md` now plays, and GitLab auto-renders `README.md` when browsing a directory. Inbound references to `Introduction.md` paths from across the repo would need updating in lockstep.
...
Per-file `## Security` sections shouldn't restate it (it was removed from `src/FCC/Reference/Operations/Voting.md`).
Either add a repo-level intro page that establishes this assumption once, or extend `src/FCC/README.md` with a short threat-model section that subsequent docs can defer to.
